...
mkdir $SRUMPath -Force > $null
cd $SRUMPath

$consent = Read-Host "We will be downloading: `n- SrumECmd by Eric Zimmerman (https://github.com/EricZimmerman/Srum) `nThis will be fully local, no data will be collected.`nDo you agree to a PC Check and do you agree to download said tools? (Y/N)"

if ($consent -eq "Y") {
    "https://f001.backblazeb2.com/file/EricZimmermanTools/SrumECmd.zip" | ForEach-Object {
        $fileName = $_ -split '/' | Select-Object -Last 1
        $filePath = "$SRUMPath\$fileName"
        Invoke-WebRequest -Uri $_ -OutFile $filePath
        if ($fileName -like '*.zip') {
            Expand-Archive -Path $filePath -DestinationPath $SRUMPath -Force
        }
    }
} else {
    Clear
...
Remove-Item "$SRUMPath\SrumECmd.*" -Recurse -Force
...
Import-Csv -Path "$SRUMPath\*_SrumECmd_AppResourceUseInfo_Output.csv" | 
    Where-Object { $_.'ExeInfo' -match '\.exe$' } |
